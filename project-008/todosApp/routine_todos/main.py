...
from routine_todos.database import engine, get_session
from routine_todos.models import Todo, Todo_Created, Todo_Edited
...
app: FastAPI = FastAPI(
    lifespan = lifespan, 
    title = "Routine_Todos API with DB, Docker Compose and PgAdmin", 
    description = "This is an API of Todos, integrated with DB, Docker Compose and PgAdmin", 
    version = "0.1.5",
    servers=[
        {
...
@app.get("/")
async def root():
    return {"message": "Welcome to Routine_Todos App with DB, Docker Compose and PgAdmin"}

@app.post("/todos/", response_model=Todo)
async def create_todos(todo: Todo_Created, session: Annotated[Session, Depends(get_session)]):
...
@app.put("/todos/{id}")
async def edit_todo(todo: Todo_Edited, id: int, session: Annotated[Session, Depends(get_session)]):
