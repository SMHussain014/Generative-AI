from kafka_producer.settings import BOOTSTRAP_SERVER, KAFKA_TOPIC
...
async def create_kafka_topic():
    admin_client = AIOKafkaAdminClient(bootstrap_servers=str(BOOTSTRAP_SERVER))
    retries = 0
    while retries < MAX_RETRIES:
        try:
            await admin_client.start()
            topic_list = [NewTopic(name=str(KAFKA_TOPIC), num_partitions=1, replication_factor=1)]
            try:
